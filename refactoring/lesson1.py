...
def get_trees(_path, with_filenames=False, with_file_content=False):
    ...
    filenames = [os.path.join(dirname, file) for dirname, dirs, files in os.walk(path, topdown=True) for file in files if file.endswith('.py') if len(filenames) <= 100]
    ...
    for filename in filenames:
        ...
        if with_filenames:
            ...
            # рефактор
            trees.append((filename, main_file_content, tree)) if with_file_content else trees.append((filename, tree))
...
if __name__ == '__main__':
    ...
    top_size = 200
    logger.info(f'total {len(wds)} words, {len(set(wds))} unique')
    # for project in projects:
    #     path = os.path.join('.', project)
    #     wds += get_top_verbs_in_path(path)
    # top_size = 200
    ...
    # ref
    [logger.info(word, occurence) for word, occurence in collections.Counter(wds).most_common(top_size)]
